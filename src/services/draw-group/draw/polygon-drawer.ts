...
export interface IPolygonDrawerOptions extends IPolygonModeOptions<IPolygonData> {
  liveUpdate: boolean;
}

export class PolygonDrawer extends PolygonMode<IPolygonDrawerOptions> {
  constructor(CoreService: ICoreService, options: DeepPartial<IPolygonDrawerOptions>) {
  ...
  removeNode(node: IPointData | string, feature: IPolygonData | string) {
    const targetFeature = this.getTargetData(feature) as IPolygonData | undefined;
