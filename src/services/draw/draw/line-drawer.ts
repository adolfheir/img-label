...
export class LineDrawer extends LineMode {
  ...
  protected getHelper() {
    return cloneDeep(DEFAULT_LINE_HELPER_CONFIG);
  }
  ...
  removeNode(node: IPointData | string, feature: ILineData | string) {
    const targetFeature = this.getTargetFeature(feature) as ILineData | undefined;
