...
import type { IFaceService, Point, Rect } from './interface';
import { FaceEvent } from './interface';

@injectable()
export class FaceService extends EventEmitter<`${FaceEvent}`> implements IFaceService {
  ...
  handleClick(event: FederatedPointerEvent) {
    ...
    if (conatiner && this.able) {
      ...
      if (this.pointList.length == 5) {
        ...
        //下一帧发 保证点位能先看到
        setTimeout(() => {
          this.emit(FaceEvent['onFaceDrawDone']);
        }, 16);
      }
    }
  }
